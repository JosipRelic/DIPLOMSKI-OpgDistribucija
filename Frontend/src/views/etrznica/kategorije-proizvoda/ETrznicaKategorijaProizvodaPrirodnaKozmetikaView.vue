<template>
  <section>
    <KategorijeProizvodaProizvodiNaslovna
      nazivKategorije="Prirodna kozmetika"
      opis="Balzami, kreme, sapuni i ulja na bazi prirodnih sastojaka. Ručno rađeni proizvodi za njegu kože bez štetnih kemikalija."
      :slika="slikeETrznicaKategorijeKozmetika"
    />
  </section>
  <section>
    <div class="mx-auto max-w-screen-xl px-4 py-8 sm:px-6 sm:py-12 lg:px-8">
      <div class="mt-4 lg:mt-8 grid grid-cols-1 lg:grid-cols-4 gap-8">
        <KategorijeProizvodaProizvodiFilteri />

        <div
          class="lg:col-span-3 grid grid-cols-1 max-sm:grid-cols-2 sm:grid-cols-2 md:grid-cols-3 gap-6 px-4 py-8 mb-14"
        >
          <KarticaProizvoda />
          <Paginacija />
        </div>
